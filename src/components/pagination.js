...
const Pagination = ({ currentPage, totalPages, paginate }: Props): Node => {
  ...
  return (
    <div className="pagination">
      ...
      <Button onClick={() => getPrevious(current)} disabled={showPreviuos}>
        {previous}{' '}
      </Button>
      <span>{`${page} ${current} ${of} ${totalPages}`}</span>
      <Button onClick={() => getNext(current)} disabled={showNext}>
        {next}
      </Button>
